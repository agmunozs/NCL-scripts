...
c ------------------------------------------------------------------
C NCLFORTSTART
      SUBROUTINE DCAPECALC3D(PRS,TMK,QVP,GHT,TER,SFP,CAPE,CIN,NCAPE,
     +                       NCIN,MIY,MJX,
...
c
      ...
      DOUBLE PRECISION NCAPE(MIY,MJX,MKZH)
      DOUBLE PRECISION CIN(MIY,MJX,MKZH)
      DOUBLE PRECISION NCIN(MIY,MJX,MKZH)
      CHARACTER*(*) PSAFILE
C NCLEND
c Local variables
...
c
		  ...
		  NCAPE(I,J,KPAR) = CAPE(I,J,KPAR)/(ZREL(KEL)-ZREL(KLFC))
		  NCIN(I,J,KPAR) = CIN(I,J,KPAR)/(ZREL(KLFC)-ZREL(1))
c
